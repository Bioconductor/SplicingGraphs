...
a gene model is typically represented as a \Rclass{TxDb}.
...
% NOTE: The lapply() loop below should take less than 1 min on a modern laptop
% when the vignette is passed thru Sweave(), because in that case, 'sg' already
% contains all the precomputed bubbles in 'sg@.bubble_cache'. With a fresh 'sg'
% however, it would take between 6 and 7 min on a modern laptop.
\begin{small}
<<AScode_summary>>=
