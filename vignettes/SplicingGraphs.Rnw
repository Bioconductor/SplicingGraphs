...
%\VignetteDepends{SplicingGraphs,TxDb.Hsapiens.UCSC.hg19.refGene,RNAseqData.HNRNPC.bam.chr14,Gviz}
...
\date{Last modified: November 2025; Compiled: \today}
...
Figure~\ref{img:LINC01533} shows the splicing graph representation of
all known transcript variants of Human gene LINC01533 (Entrez ID 100996665).

\begin{figure}[!htbp]
  \begin{center}
  \includegraphics[width=5in,height=2.25in,angle=-90,origin=rB]{images/LINC01533-transcripts.pdf}
  \hskip 0.5in
  \raisebox{-0.25in}{\includegraphics[width=2in,height=5.5in]{images/LINC01533-sg.pdf}}
  \end{center}
  \caption{Splicing graph representation of the three transcript variants
           of gene LINC01533 (Entrez ID 100996665).
...
An example of such a splicing graph is shown in Figure~\ref{img:ZNF446}.

\begin{figure}[!htbp]
  \begin{center}
  \includegraphics[width=5.5in,height=2in,angle=-90,origin=rB]{images/ZNF446-transcripts.pdf}
  \hskip 0.5in
  \raisebox{-0.6in}{\includegraphics[width=2in,height=6.5in]{images/ZNF446-sg.pdf}}
  \end{center}
  \caption{Splicing graph representation of the two transcript
           variants of Human gene ZNF446 (Entrez ID 55663).
           Node $3$ is a (\Rcode{\AcceptorSite},\Rcode{\TSSite}) dual-type
           node: \textit{acceptor} (\Rcode{\AcceptorSite})
           for transcript {\tt NM\_017908}, and \textit{transcription start}
           (\Rcode{\TSSite}) for transcript {\tt NM\_001304453}.
  }
  \label{img:ZNF446}
\end{figure}
...
illustrated in Figure~\ref{img:LINC01533-simplified}.

\begin{figure}[!htbp]
  \begin{center}
  \includegraphics[width=2in,height=5.5in]{images/LINC01533-sg.pdf}
  \hskip 0.5in
  \includegraphics[width=2in,height=5.5in]{images/LINC01533-sg2.pdf}
  \end{center}
  \caption{Nodes \Rcode{1}, \Rcode{3}, \Rcode{4}, \Rcode{7}, \Rcode{9},
           \Rcode{10}, \Rcode{11}, \Rcode{12}, and \Rcode{13}, are
           uninformative and can be removed.
           Left: before removal of the uninformative nodes.
           Right: after their removal.
  }
  \label{img:LINC01533-simplified}
\end{figure}
...
repositories. Here the \Rpackage{TxDb.Hsapiens.UCSC.hg19.refGene}
...
\Rfunction{lengths} is a core accessor for list-like objects
that returns the lengths of the individual list elements.

At this point you might wonder why \Rfunction{lengths} works on
\Rclass{SplicingGraphs} objects. Does this mean that those objects
are list-like objects? The answer is yes.
...
we need to extract them explicitly:
...
\begin{figure}[!htbp]
  ...
  \end{center}
  \caption{The 4 transcripts of gene HNRNPC (Entrez ID 3183).}
...
way to select all the elements with some given names is to do something
like \Rcode{x[names(x) \%in\% c("name1", "name2")]}.
For example, to select all the transcripts from genes 10001 and 100128927:
...
We start by introducing the notion of a valid path between two nodes of a
splicing graph: a ``valid path'' between two nodes $s$ and $t$ ($s < t$ )
of a splicing graph is a path followed by at least one transcript (see the
\Rfunction{txpath} function in previous section for how to extract the
transcript paths).

Let's illustrate this by looking at the splicing graph for gene 115801414
(Entrez ID). This gene is made of 3 transcripts: \Rcode{NR\_164308},
\Rcode{NR\_164309}, and \Rcode{NR\_164310}. Its splicing graph is shown
on figure~\ref{img:115801414-sgraph}.

<<plot_sg_115801414_as_pdf,echo=FALSE,results=hide>>=
...
\begin{figure}[!htbp]
  \begin{center}
  \includegraphics[width=5in,height=5in]{115801414-sgraph-labelled.pdf}
  \end{center}
  \caption{Splicing graph representation of gene 115801414 (Entrez ID).
           Valid paths between nodes \Rcode{2} and \Rcode{14} are:
           \Rcode{\{2,7,8,14\}} (followed by
           transcript \Rcode{NR\_164308}),
           \Rcode{\{2,9,11,12,13,14\}} (followed by
           transcript \Rcode{NR\_164309}), and
           \Rcode{\{2,3,4,5,6,7,8,10,11,12,13,14\}} (followed by
           transcript \Rcode{NR\_164310}).
           Paths \Rcode{\{2,3,4,5,6,7,8,14\}} or
           \Rcode{\{2,7,8,10,11,12,13,14\}} are not followed by
           any known transcript and thus are not valid paths.
  }
  \label{img:115801414-sgraph}
\end{figure}

Path \Rcode{\{2,7,8,14\}} in that graph is followed
by transcript \Rcode{NR\_164308} and thus is valid. However path
\Rcode{\{2,3,4,5,6,7,8,14\}} is not followed by any known transcript
and thus is not valid.

Now the definition of a bubble: there is a ``bubble'' between two nodes $s$
and $t$ ($s < t$ ) of a splicing graph if (a) there are at least two distinct
...
For example, there is a bubble between nodes \Rcode{2} and \Rcode{14}
of gene 115801414 (see figure~\ref{img:115801414-sgraph}) because there are
3 valid paths between the two nodes and these paths share those two nodes
only. The dimension of this bubble is 3.
However, there is no bubble between nodes \Rcode{2} and \Rcode{L}
because the 3 valid paths between the two nodes also share nodes
\Rcode{14}, \Rcode{15}, \Rcode{16}, \Rcode{17}, \Rcode{18}, and \Rcode{19}.
...
Let's compute the bubbles for gene 115801414 (Entrez ID) (represented on
figure~\ref{img:115801414-sgraph}):

<<bubbles_sg_115801414>>=
...
bubbles of smaller (or higher) dimensions. For example, in gene 115801414,
the bubble between nodes \Rcode{2} and \Rcode{14} (3rd bubble in the
\Rclass{DataFrame} object returned by \Rfunction{bubbles}) is of
dimension 3 and contains the smaller bubble between nodes \Rcode{8}
and \Rcode{14} (4th bubble in the \Rclass{DataFrame} object returned
...
Let's illustrate this by looking at how the AS code for the 1st bubble
in gene 115801414 is obtained. In 3 steps:
\begin{enumerate}
  \item The paths reported for this bubble are \Rcode{\{\}} and
        \Rcode{\{3,4,5,6\}}. All the nodes involved in these paths
        are re-numbered starting at \Rcode{1} so nodes \Rcode{3}, \Rcode{4},
        \Rcode{5}, \Rcode{6} become \Rcode{1}, \Rcode{2}, \Rcode{3}, \Rcode{4},
        respectively.
  \item Then, for each path, a string is generated by putting together
        ...
        \Rcode{\{\}} and \Rcode{\{3,4,5,6\}} generate strings \Rcode{0}
        and
        \Rcode{1{\AcceptorSite}2{\DonorSite}3{\AcceptorSite}4{\DonorSite}},
        respectively.
  \item Finally, the strings obtained previously are sorted
        ...
        \Rcode{0,1{\AcceptorSite}2{\DonorSite}3{\AcceptorSite}4{\DonorSite}}.
\end{enumerate}
...
\Rcode{"2 alternative first exons"}, which indicates that the event
is not internal. This raises at least 2 questions: why is it
...
% With a fresh 'sg' however, it would take about 1 min on a modern laptop.
% This can be reduced by replacing lapply() with BiocParallel::bplapply().
\begin{small}
<<AScode_summary>>=
...
At this point it should be mentioned that not all splicing events have
descriptions in the English language since there are a variety of different
events.
...
Next we want to see which genes show the highest heterogeneity of
splicing events.
...
Below the genes with the highest heterogeneity in observed splicing
events are shown.
...
<<plot_sg_91833_as_pdf,echo=FALSE,results=hide>>=
...
\begin{figure}[!htbp]
  \begin{center}
  \includegraphics[width=5in,height=5in]{91833-sgraph.pdf}
  \end{center}
  \caption{Splicing graph representation of gene WDR20 (Entrez ID 91833).
           This is the gene with the most observed splicing events on
           Human chromosome 14.}
  \label{img:91833-sgraph}
\end{figure}
...
\begin{figure}[!htbp]
  ...
  \end{center}
  \caption{Plot of the genomic region chr14:21675000-21702000 showing the 4
