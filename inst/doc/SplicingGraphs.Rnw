...
\Rfunction{readGappedAlignments} function from the \Rpackage{GenomicRanges}
...
gal <- readGappedAlignments(bam_files[1], param=param)
length(gal)  # nb of BAM records
head(gal)
@

The result is a \Rclass{GappedAlignments} object with 1 element per BAM
record. Although paired-end reads can also be loaded with
this function, they should preferably be loaded with the
\Rfunction{readGappedAlignmentPairs} function (also from the
\Rpackage{GenomicRanges} package): this will actually pair
the BAM records and return the pairs in a \Rclass{GappedAlignmentPairs}
...
that we'll pass to \Rfunction{readGappedAlignments} (see
...
SRR316184gal <- readGappedAlignments(bam_files[1], use.names=TRUE, param=param0)
