...
\cite{Anders:2010:GB}, the approach of analyzing splice events as
...
the full variety of splice variants of a certain gene in one single 
data structure Heber at al \cite{Heber01072002} introduced the term 
splice graph and provided a formal frame work for representing
...
acceptor splice site ($-$), donor splice site ($\wedge$), transcript start 
site ($~[~$) or transcript end ($~]~$).  For the correctness of the splicing 
graph framework artificial sites designated as root ($0$) and leaf ($1$) 
...
transcripts and act just as a common sources and sink of the splice graph. 
The symbols associated with the individual types of vertices shown in braces 
above are used for the identification of complete alternative splice events.
For details about gathering the alternative splice events and their
associated splice codes see \cite{Heber01072002} and 
\cite{Sammeth:2009:GB}.

Figure \ref{img:SplicingGraphs} shows the splice graph representation
of all known transcript variants of gene JUNB (ENTREZ gene ID 3726).
...
The green dashed lines in Figure \ref{img:SplicingGraphs}  represent
genomic locations where exons start, end or get disjoined during the 
splicing graph construction process. Due to disjoining new exons are 
created in such a way that none of this new exons overlap each other.
...
\begin{figure}[H]
  ...
  \caption{Splicing graph representation of the two transcript variants
    of the gene 3726 (JUNB) resulting in 2 bubbles. Left: splicing graph
    representation. Right: transcript representation
  }
  \label{img:SplicingGraphs}
\end{figure}


\subsection{Advantages of disjoining exons}

Counting the reads landing on the transcript annotations of a certain 
reference genome is a critical step in each RNA-Seq transcriptome analysis.
...
counting for this rather short sequences could be problematic.
...
\Rpackage{SplicingGraphs} and \Rpackage{DEXSeq} avoid the problem of counting 
reads landing on overlapping exons of different transcripts by
disjoining the exons per genes into non-overlapping parts.

Regrading the splicing graph structure disjoining of exons is revealing new
sites at the transcripts flagged with a $:$ symbol. Each site where disjoining 
occurs can be considered as an intron of zero length
flanked by two vertices, one representing the
end of the first exon part (${\wedge}{:}$) and one representing the start of 
the second exon part (${-}{:}$). See also Figure \ref{img:SplicingGraphs}.
...
Not all splice sites (vertices) on the individual transcripts are alternative 
splice sites. Therefore the initial splicing graph outlined above can be simplified 
by collapsing vertices of in- and out- degree equal to one because they
are supposed to be non-informative in terms of alternative splicing.
...
splicing. Between two remaining vertices there exist at
least two edges which form a bubble. Such a bubble, representing an alternative
splice event, consists at least of two bubble parts. The exonic structures of the 
two bubble parts are exclusive.
Bubbles in general can be nested or can intersect with other bubbles.
The collapsed edges of the final splicing graph associated with one or more exons 
which do not overlap each other, form larger sequence regions providing 
advantages in counting reads and testing for differential expression. 
...
the splice code ($2~1 - 2~2~\wedge$) represents edge number 2 
...
\begin{figure}[H]
  ...
  \caption{Splice graph representation of an 1 exon skip leading to
    one bubble. The associated splice code is 
    $1~0~0~2~1 - 2~2~\wedge$. Left: splice graph
    representation. Right: transcript representation
  }
  \label{img:exskip}
\end{figure}
...
The code for an alternative splice site acceptor would be 
...
To run the \Rfunction{spliceGraphs} function only on a subset of
...
Finally we can execute the \Rfunction{spliceGraphs} function onto 
the modified \Rclass{TranscriptDb} object.

<<loadGenomicFeatures>>=
library(SplicingGraphs)
sG <- spliceGraphs(txdb, genes="20469")
@
...
advantages of the splice graph approach the reads for the 
...
provided in the \Rpackage{RNASeqDataSubset}.
...
if(reCnt) {
  library(parallel)
  cD.exByEdge <- 
    countSpliceGraphReads(sG, bfs)
  save(cD.exByEdge, file="cD.exByEdge.Rda")
} else {
  fn <- system.file("extdata", "cD.exByEdge.Rda", 
                    package="RNASeqDataSubset")
  load(fn)
}
@ 
...
\section{Significant altered alternative splice events}

This section deals with testing for significant altered splice events.
We introduced a generalized linear model (GLM) to asses the
significance of changes in the alternative splice events between
...
splice graph, exon terms are replaced by the bubble part terms. 
...
A bubble part in the simplest case consist of only one splice graph edge.
...
<<testingForSignificantAlteredSpliceEvents>>=
...
<<accessingCounDataAndExonCountData, results=hide>>=
...
alternative splice events to be significantly (p $<$ 0.05) altered.

<<printSigSpliceEvents, results=tex, echo=FALSE>>=
library(xtable)
T <- xtable(head(res.bubbles, n=10),
             caption=paste("Top 10 most significant altered alternative splice events"))
