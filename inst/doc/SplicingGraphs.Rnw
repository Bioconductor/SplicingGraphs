...
Figure \ref{img:SplicingGraphs} shows the splicing graph representation
of all known transcript variants of gene JUNB (ENTREZ gene ID 3726).
Before the splicing graph is constructed the exons of the individual genes
...
simplified by collapsing vertices of in- and out- degree equal to one because
...
splicing. Between two remaining vertices there exists at
least two edges which form a bubble. Such a bubble, representing an
alternative splicing event, consists at least of two bubble parts. The exonic
structures of the two bubble parts are exclusive.
Bubbles in general can be nested or can intersect with other bubbles.
The collapsed edges of the final splicing graph associated with one or more
exons which do not overlap each other, form larger sequence regions providing 
...
The types of alternative splicing events are provided as a \Rclass{DataFrame}
object. The first column provides the so-called bubble code. The
bubble code assigns an unique identifier to the individual splicing events
(bubbles) and consists basically of an bubble ID concatenated with the
ENTREZ gene ID. The second column provides
\Rclass{CompressedCharacterList}s representing the splicing codes.
To explore the meaning of the code symbols go back to Section \ref{sec:spliceCodes}.
The last three columns provide the name of the splicing event, the
corresponding ENTREZ gene ID and the bubble ID unique within each gene.

Back to quantifying the observed splicing events in the 
annotation. Now we are interested which types of splicing events occur most
often.
...
The most often occurring splicing event on chromosome 19 is named skip
one exon. At this point it should be mentioned that not all splicing
...
graph is how many splicing events does each individual gene have.
