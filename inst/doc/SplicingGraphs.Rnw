...
Two artificial nodes that don't represent real observed splicing sites
...
The non-artifical nodes (i.e. $1$ to $n$) and edges are the
\textit{internal nodes} and \textit{internal edges} of the splicing graph.
...
Internal nodes can be of the following types:
\begin{itemize}
  \item acceptor splicing site ($-$);
  \item donor splicing site ($\wedge$);
  \item transcript start site ($~[~$);
  \item transcript end site ($~]~$).
\end{itemize}

Wether the edge is representing an exon or an intron is determined by
the types of the two flanking nodes.
...
In many cases one node can represent two different types of sites, but
the combinations of types for such nodes is limited. 
...
Such a two-type node occurs when the start of an exon, which is not
...
\begin{figure}[tbp]
  ...
  \caption{Splicing graph representation of the two transcript variants
    of Human gene ZNF813 (Entrez ID 126017). Node $3$ is a two-type node:
...
depending on the types of the flanking nodes.
An edge between a splicing site acceptor ($-$) and a splice site donor
($\wedge$) node for example would represent an exon whereas the edge
...
ends ($~]~$). The edges between the root node ($R$) and the transcription
...
The final splicing graph only contains nodes involved in alternative
