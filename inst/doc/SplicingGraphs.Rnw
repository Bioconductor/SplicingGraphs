...
gene 329047 because the 2 valid paths between them don't share other nodes.
The dimension of this bubble is 2.
...
Let's illustrate this by looking at how the code for the 2nd bubble
...
\begin{enumerate}
  ...
  \item Then, for each path, a string is generated by putting together
        the node numbers and types for the path. By convention, this string
        is set to \Rcode{0} if the path is empty. So path \Rcode{\{\}}
        and \Rcode{\{5,6\}} generate strings \Rcode{0} and \Rcode{0,1-2\^},
        respectively.
  \item Finally, the strings obtained previously are sorted in
        lexicographical order and concatenated together separated
        by commas (\Rcode{,}).
\end{enumerate}
...
\Rcode{ASCODE2DESC} only contains the 50 code descriptions reported in
Table $1$ of Sammeth's paper \cite{Sammeth:2010}. Therefore looking
up codes thru \Rcode{ASCODE2DESC} returns \Rcode{NA} for some codes.
Even a simple code like \Rcode{0,1[2\^} is not given a code in Sammeth's
paper \cite{Sammeth:2010} (would be \Rcode{"skip first exon"} if it was),
because, for some reasons, Table $1$ in the paper is only considering
\textbf{internal} complete events (as its title indicates), that is,
bubbles with source and sink nodes that are not \Rcode{R} and \Rcode{L},
or, equivalently, complete events with a code that has no \Rcode{[} or
\Rcode{]} in it
\footnote{Strangely, Table $1$ of Sammeth's paper \cite{Sammeth:2010}
actually contains AS codes that seem to correspond to complete events
that are not internal. For example, the 16th AS code in the table is
\Rcode{1*2\^,3*4\^} and its description is \Rcode{"2 alternative first
exons"} indicates that this is not an internal complete event.
This raises at least 2 questions: (1) why is it in the table?, and
(2) why \Rcode{[} has been replaced with \Rcode{*} in the code?
A 3rd question could be: why exclude non-internal complete events
