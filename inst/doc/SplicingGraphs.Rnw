...
\date{Last modified: April 2013; Compiled: \today}
...
The study, performed on \textit{Mus musculus}, compares TBX20 (T-box
protein 20) knock-out with wilde type heart tissue samples \cite{Sakabe:2012}.
...
The resulting plot is shown in figure~\ref{img:107328-transcripts}.
...
The resulting plots are shown in figure~\ref{img:107328-sgraph}.
...
(Entrez ID). The graph is shown in figure~\ref{img:329047-sgraph}.
...
\begin{enumerate}
  \item The paths reported for this bubble are \Rcode{\{\}} and
        \Rcode{\{5,6\}}. All the nodes involved in these paths are
...
\Rpackage{TBX20BamSubset} package. They've been aligned to the \textit{mm9}
reference genome from UCSC, which is also the reference genome of the gene
model in our \Rclass{SplicingGraphs} object \Rcode{sg}. As a general rule,
the aligned reads and the annotations should always be based on the same
reference genome.

Let's start by loading the \Rpackage{TBX20BamSubset} package:

<<bam_files>>=
library(TBX20BamSubset)
bam_files <- getBamFileList()
names(bam_files)  # the names of the samples
@
...
Doing this for the 6 BAM files confirms that all the reads in the
files are single-end. This means that they should be loaded with the
\Rfunction{readGAlignments} function from the \Rpackage{GenomicRanges}
package. As a quick example, we load the reads located on the first 3.2
million bases of chromosome 19:

<<readGAlignments>>=
param <- ScanBamParam(which=GRanges("chr19", IRanges(1, 3200000)))
gal <- readGAlignments(bam_files[1], param=param)
length(gal)  # nb of BAM records
head(gal)
@

The result is a \Rclass{GAlignments} object with 1 element per BAM
record. Although paired-end reads can also be loaded with
this function, they should preferably be loaded with the
\Rfunction{readGAlignmentPairs} function (also from the
\Rpackage{GenomicRanges} package): this will actually pair
the BAM records and return the pairs in a \Rclass{GAlignmentPairs}
object. Otherwise they won't be paired and will be returned in a
...
We're going to loop on the BAM files to assign the reads from 1 sample
...
that we'll pass to \Rfunction{readGAlignments} (see
...
for (i in seq_along(bam_files)) {
    ...
    gal <- readGAlignments(bam_file, use.names=TRUE, param=param0)
...
<<sg_107328_assignments>>=
edges_by_tx <- sgedgesByTranscript(sg["107328"], with.hits.mcols=TRUE)
edge_data <- mcols(unlist(edges_by_tx))
colnames(edge_data)
@

There is 1 \textit{hits} column per sample. They appear in the
order they were created. Each of them is a \Rclass{CharacterList}
object that contains the reads from that sample that were assigned
to the individual edges. Let's look at the \textit{hits} column for
the first sample (\Rcode{SRR316184}):

<<head_edge_data_sgedge_id_SRR316184.hits>>=
head(edge_data[ , c("sgedge_id", "SRR316184.hits")])
@
...
For the purpose of explaining how reads from sample \Rcode{SRR316184}
were assigned to the edges of gene 107328, we load the reads that are
in the region of that gene:

<<load_reads_in_107328_region>>=
SRR316184gal <- readGAlignments(bam_files[1], use.names=TRUE, param=param0)
reads <- subsetByOverlaps(SRR316184gal, range(sg[["107328"]]),
                          ignore.strand=TRUE)
@

and we plot them:

<<plotTranscripts_sg_107328_and_reads,eval=FALSE>>=
plotTranscripts(sg[["107328"]], reads=reads)
@

The resulting plot is shown on figure \ref{img:107328-transcripts-and-reads}.

<<plotTranscripts_sg_107328_and_reads_as_pdf,echo=FALSE,results=hide>>=
pdf("107328-transcripts-and-reads.pdf", width=6, height=3)
plotTranscripts(sg[["107328"]], reads=reads)
dev.off()
@
\begin{figure}[!htbp]
  \begin{center}
  \includegraphics[width=6in,height=3in]{107328-transcripts-and-reads.pdf}
  \end{center}
  \caption{The 3 transcripts of gene 107328 (Entrez ID) and the reads
           in that region.}
  \label{img:107328-transcripts-and-reads}
\end{figure}

[TODO: To be continued. But first we would need to use different RNA-seq
 data. This one is boring: it contains no junction reads (no N's in the
 CIGARs) so is useless for detecting splicing sites and/or assigning reads
 to intronic edges (aka junctions).]

\subsection{Counting and summarizing the assigned reads}

We use \Rfunction{countReads} to count the number of reads assigned
to each (non-artificial) edge in \Rcode{sg} for each sample:
