...
\subsection{Choose and load a gene model}
...
\subsection{Generate a \Rclass{SplicingGraphs} object}
...
\begin{enumerate}
  \item \Rcode{exon\_id}:
  \item \Rcode{exon\_name}:
  \item \Rcode{exon\_rank}:
  \item \Rcode{start\_SSid}:
  \item \Rcode{end\_SSid}:
\end{enumerate}

The \Rfunction{show} method for \Rclass{GRangesList} objects only
displays the inner metadata columns (which are at the exon level in
the case of \Rcode{sg[["107328"]]}). To see the outer metadata columns
(transcript-level metadata columns in the case of \Rcode{sg[["107328"]]}),
...
The transcripts in \Rcode{sg[["107328"]]} can be plotted with
\Rfunction{plotTranscripts}:
