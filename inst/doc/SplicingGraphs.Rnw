...
[TODO: Revisit this sub-section entirely.]
...
<<eval=FALSE>>=
AScode_count <- sort(table(unlist(lapply(seq_along(sg),
                                  function(i) bubbles(sg[i])$AScode))),
                     decreasing=TRUE)
AScode_table <- data.frame(AScode=names(AScode_count),
                           NbOfEvents=as.vector(AScode_count),
                           Desciption=SplicingGraphs:::AScode2desc[names(AScode_count)])
@

<<>>=
head(AScode_table, n=10)
@
