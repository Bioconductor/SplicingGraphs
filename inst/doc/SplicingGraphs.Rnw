...
The package defines the \Rclass{SplicingGraphs} container for storing
the splicing graphs together with the gene model that they are based on.
Several methods are provided for conveniently access the information
stored in a \Rclass{SplicingGraphs} object. Most of these methods are
...
Finally, in this section, we illustrate how RNA-seq reads can be assigned
...
Another important information about RNA-seq reads is whether they are
...
Doing this for the 6 BAM files would confirm that all the reads in the
...
record. Note that, although paired-end reads can also be loaded with
...
the exonic and intronic edges of \Rcode{sg}. Note that the same read
...
We loop on the BAM files to assign the reads from 1 sample at a time:
...
All the assignments can be retrieved by calling either
\Rfunction{sgedgesByGene} or \Rfunction{sgedgesByTranscript} on
\Rcode{sg} and with \Rcode{with.hits.mcols=TRUE}. Here we only
show how \Rfunction{sgedgesByGene} can be used for this:

<<sgedgesByGene_sg_with_hits>>=
edges_by_gene <- sgedgesByGene(sg, with.hits.mcols=TRUE)
sg_assignments <- mcols(unlist(edges_by_gene))[ , -(2:5)]
@

\Rcode{sg\_assignments} is a \Rclass{DataFrame} with 1 row per
(non-artificial) edge in \Rcode{sg}, and the following columns:

<<sg_assignments>>=
colnames(sg_assignments)
@

There is 1 \textit{hits} column per sample. Each of them is a
\Rclass{CharacterList} object that contains the reads from that
sample that were assigned to the individual edges.

\subsection{Counting and summarizing the assigned reads}

Finally we use \Rfunction{countReads} to count the number of reads assigned
