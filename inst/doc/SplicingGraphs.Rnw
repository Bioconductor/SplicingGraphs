...
function on the gene model. This is the constructor function for the
\Rclass{SplicingGraphs} class. It will compute information about all
the splicing graphs (1 per gene) and store it in the returned
\Rclass{SplicingGraphs} object.
...
\subsection{Extracting information from a \Rclass{SplicingGraphs} object}

The transcripts of a given gene can be extracted with \Rfunction{[[}.
The result is an \emph{unnamed} \Rclass{GRangesList} object containing
...
\begin{itemize}
  \item \Rcode{exon\_id}: The original internal exon id as stored in the
        ...
        It's not a public id (like e.g. an Ensembl, RefSeq, or GenBank id).
        and is only guaranteed to be unique within a \Rclass{TranscriptDb}
        object.
  ...
  \item \Rcode{start\_SSid}, \Rcode{end\_SSid}: The ids of the splicing
        sites corresponding to the start and end positions of the exon.
        Those ids were assigned by the \Rfunction{SplicingGraphs} constructor.
\end{itemize}

As mentioned previously, the splicing sites ids are assigned based on their
order of appearance when moving in the 5' to 3' direction along the strand
of the gene. This means that, for a gene on the minus strand, the ids in
the \Rcode{start\_SSid} metadata column are greater than those in the
...
Note that the list elements of the \Rcode{txpaths} metadata column are
always made of an even number of increasing splicing site ids.
...
\subsection{Plotting a splicing graph}
