...
\begin{itemize}
  ...
  \item \Rcode{AScode}, \Rcode{description}: The AS code (Alternative Splicing
        code) and its description. More on these in the next subsection.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Then, for each path, a string is generated by putting together
        ...
        respectively.
  \item Finally, the strings for all the paths are concatenated together
        separated by a comma (\Rcode{,}).
\end{enumerate}
...
named character vector that contains the descriptions for the AS codes of
the 50 most frequent patterns of internal complete events found in Human
as reported in Table $1$ of Sammeth's paper \cite{Sammeth:2010}.

[TODO: Revisit stuff below]
