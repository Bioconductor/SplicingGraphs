...
and return the pairs in a \Rclass{GAlignmentPairs} object. Otherwise they
won't be paired and will be returned in a \Rclass{GAlignments} object.
...
## The following loop should take about 7-8 minutes...
...
pdf("3183-transcripts-and-reads.pdf", width=6, height=3)
plotTranscripts(sg[["3183"]], reads=gapped_reads, from=21675000, to=21702000)
dev.off()
@
\begin{figure}[!htbp]
  \begin{center}
  \includegraphics[width=6in,height=3in]{3183-transcripts-and-reads.pdf}
  \end{center}
...
to each (non-artificial) edge in \Rcode{sg} for each run:
